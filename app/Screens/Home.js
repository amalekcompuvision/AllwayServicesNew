...
const Home = (props) => {
  ...
  let startShiftBody =
    language == 'EN'
      ...
      : 'هل بالتأكيد تريد بدأ المناوبة؟';
  let endShiftBody =
    language == 'EN' ? 'End Shift ?' : 'هل بالتأكيد تريد انهاء المناوبة؟';
  ...
  const setCounters = async (id) => {
    ...
    action = driverInfoActions.setDriverOrderCount(id);
    try {
  ...
  const setDriverStatus = async (id, status) => {
    ...
    action = driverInfoActions.setDriverStatus(id, status);
    try {
      await dispatch(action);
      // console.log('set counter started working from homescreen');
    } catch (err) {
  ...
  const setDriverBigInfo = async (id) => {
    ...
    action = authActions.setDriverInfo(id);
    try {
      await dispatch(action);
  ...
  const handleSetOrders = async (driverId, status) => {
    ...
    action = ordersActions.setOrders(
      //setOrders to driverInfoActions
      driverId,
      status,
    );
    try {
  ...
  const logoutAlert = () => {
    Alert.alert(logoutAlertTitle, logoutAlertBody, [
      {
        text: No,
        onPress: () => setShouldSHowMenu(!shouldSHowMenu),
        style: 'cancel',
      },
      {
        text: Yes,
    ...
    <View style={styles.container}>
      ...
      {shouldSHowMenu ? (
        <View
          ...
          <Menu.Item
            ...
            onPress={() => {
              Alert.alert(changeLangAlertTitle, changeLangAlertBody, [
                {
                  text: No,
                  onPress: () => setShouldSHowMenu(!shouldSHowMenu),
                  style: 'cancel',
      ...
      <ScrollView
          ...
          <ButtonBlock
            style={[
              styles.Block,
              {
                ...
                backgroundColor:
                  driverInfoFromRedux.status == 0 ? 'red' : 'green',
              },
            ]}
        ...
        <Block
          ...
          title={'chats'}
        ></Block> */}
          <Block icon={'book'} title={Stats}></Block>
        </View>
